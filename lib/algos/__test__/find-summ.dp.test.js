const findSumm = require('../find-summ.dp');
...
describe('findSumm', () => {
  ...
  it('should find subset in a given array', () => {
    const items = [1, 5, 8, 10, 17];
    expect(
      findSumm(items, 19)
    ).toEqual([1, 8, 10]);
  ...
  it('should find subset in a given unsorted array', () => {
    const items = [17, 5, 1, 10, 8];
    expect(
      findSumm(items, 19)
    ).toEqual([1, 10, 8]);
  });
  ...
  it('should return the copy of items if its total summ is target (unsorted)', () => {
    const items = [17, 5, 1, 10, 8];
    expect(
      findSumm(items, 41)
    ).toEqual([17, 5, 1, 10, 8]);
  ...
  it('should return null if target is unreachable (unsorted)', () => {
    const items = [17, 5, 1, 10, 8];
    expect(
      findSumm(items, 39)
    ).toBeNull();

...
describe('findSumm', () => {
  ...
  it('should return [58] in [1, 2, 58, 60, 70] if summ is 58', () => {
    const items = [1, 2, 58, 60, 70];
    expect(
      findSumm(items, 58)
    ).toEqual([58]);
  });

  it('should return [22, 48, 50] in [1, 22, 48, 50, 65] if summ is 120', () => {
    const items = [1, 22, 48, 50, 65];
    expect(
      findSumm(items, 120)
    ).toEqual([22, 48, 50]);
  });
